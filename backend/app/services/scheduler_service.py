"""
Scheduler service for managing automated recording schedules
"""
import asyncio
import logging
import os
...
class SchedulerService:
    """Service for managing automated recording schedules"""
    
    def __init__(self, db: AsyncSession):
        self.db = db
        self.platform_service = PlatformService(db)
        self.streamlink_service = StreamlinkService(db)
        self._running = False
    ...
    async def _load_schedules(self):
        ...
        try:
            result = await self.db.execute(
                select(RecordingSchedule).where(RecordingSchedule.enabled == True)
            )
    ...
    async def run_rotation_cleanup(self):
        ...
        try:
            ...
            result = await self.db.execute(
                select(RecordingSchedule).where(RecordingSchedule.rotation_enabled == True)
            )
            schedules = result.scalars().all()
            ...
            for schedule in schedules:
                await self._apply_rotation_policy(schedule)
                
        except Exception as e:
            logger.error(f"Error during rotation cleanup: {e}")
    
    async def _apply_rotation_policy(self, schedule: RecordingSchedule):
        ...
        try:
            ...
            result = await self.db.execute(
                ...
                .where(
                    ...
                    Recording.status != 'recording'  # Exclude active recordings
                )
    ...
    async def _monitor_recording_file_sizes(self):
        ...
        while self._running:
            try:
                # Use separate session to prevent concurrency issues
                async with AsyncSessionLocal() as session:
                    # Query only recordings with 'recording' status
            ...
            while self._running:
                try:
                    async with AsyncSessionLocal() as session:
                        ...
                        result = await session.execute(
                            select(RecordingSchedule).where(RecordingSchedule.id == schedule.id)
                        )
    ...
    async def _start_recording(self, schedule: RecordingSchedule, stream_info):
        """Start recording for a schedule (legacy method)"""
        # Create a new session for this operation
        from app.database.database import AsyncSessionLocal
        async with AsyncSessionLocal() as session:
            await self._start_recording_with_session(session, schedule, stream_info)
    ...
    async def _start_recording_with_session(self, session: AsyncSession, schedule: RecordingSchedule, stream_info):
        ...
        try:
            ...
            # Create streamlink service with this session
            streamlink_service = StreamlinkService(session)
            
            # Start recording
            success = await streamlink_service.start_recording(
            ...
            if not success:
                ...
                # Note: recording status is already updated by streamlink_service
    ...
    async def stop_monitoring(self, schedule_id: int) -> bool:
        ...
        try:
            ...
            active_recordings = await self.streamlink_service.get_active_recordings()
            ...
            if schedule_recordings:
                ...
                for recording in schedule_recordings:
                    ...
                    success = await self.streamlink_service.stop_recording(recording['id'])
                    if success:
    ...
    async def trigger_check_now(self, schedule_id: int) -> bool:
        ...
        try:
            ...
            if stream_info and stream_info.is_live:
                await self._start_recording(schedule, stream_info)
                return True
            else:
    ...
    async def get_active_recordings(self) -> List[Dict]:
        ...
        return await self.streamlink_service.get_active_recordings()
    
    async def stop_all_recordings(self) -> bool:
        """Stop all active recordings"""
        return await self.streamlink_service.stop_all_recordings()
